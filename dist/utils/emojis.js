...
if (!fs.existsSync(pendingEmojisfolderPath))
    fs.mkdirSync(pendingEmojisfolderPath, { recursive: true });
if (!fs.existsSync(uploadedEmojisFolderPath))
    fs.mkdirSync(uploadedEmojisFolderPath, { recursive: true });
export async function uploadPendingEmojis() {
    prettyLog("EMOJIS", "purple", "Uploading pending emojis...");
    const rest = new REST({ version: "10" }).setToken(process.env.BOT_TOKEN);
    async function uploadAppEmoji(emojiFile, name) {
...
export async function loadEmojis() {
    ...
    const app = await rest.get(Routes.oauth2CurrentApplication());
    const appId = app.id;
    const emojisObj = await rest.get(Routes.applicationEmojis(appId));
