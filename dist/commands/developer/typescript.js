...
export default {
    ...
    async run(message, args) {
        ...
        try {
            ...
            const fn = new AsyncFunction(...sandboxKeys, `"use strict"; return (async () => { ${code} })();`);
        ...
        const trimmedOutput = output.length > MAX_LENGTH
            ? output.slice(0, MAX_LENGTH - 3) + "..."
            : output;
        const embed = new EmbedBuilder()
            .setTitle(success ? `${emojis.success} Code Executed` : `${emojis.error} Error`)
            .addFields({ name: "Code", value: `\`\`\`ts\n${code}\`\`\`` }, { name: "Output", value: `\`\`\`ts\n${trimmedOutput}\`\`\`` })
